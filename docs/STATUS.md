...
| store/db + migrations | §6 | ✅ | 0001 init, 0002 M1 (embedding_cache/promotions), 0003 M2 (inject_ledger), 0004 observation time, 0005 fact query indexes; storage gate covers forward migrations, pragmas, corruption recovery |
...
| Retrieval & ranking | ✅ | hybrid RRF + deterministic semantic features + bounded MMR diversity: recall@1/5/10 1.00, MRR 1.00, semantic no-overlap rank 1, SessionStart broad-push includes explicit user preferences, retriever suppresses send-unsafe facts before returning results, final candidate scope filtering prevents same-session cross-workspace leaks, semantic rerank text is redacted before embedding, and fact tag updates keep FTS/vector indexes searchable. `eval retrieval` regression gate |
...
| Storage & migrations | ✅ | new `eval storage` passes 13/13: schema_version 5, reopen migrations 0, failed migrations roll back partial DDL and schema_version, v1→v5 rows preserved 3/3 with t_observed backfill and hot-path fact scope index, append-only expire tombstones retained, malformed rows skipped, missing optional ledger table degrades, WAL/FK/busy_timeout enforced, cascades/edge trail consistent |
...
measured gains are recorded in the `memory` graph. Tests: 255, gate suites: 19 (`eval all` includes run/memory/promote/drift/retrieval/gate/security/branch/temporal/conflict/procedure/mcp/storage/telemetry/provenance/resilience/benchmarks/cli-docs-demo/quality)._
