...
| Eval harness & benchmarks | ✅ | `eval benchmarks` passes 8/8: centralized 19-suite registry, A/B/C/N/S ablation confirms push 93% > pull 21% with N/S 5/5 controls, offline graphCTX-vs-Supermemory/XMem scorecard renders, live bake-off skips without key, deep local coding-memory recall is 10/10 with p95 under budget, temporal coding-memory recall and stale suppression are 5/5 with p95 under budget, 1k/10k scale p95 stays under budget, empty/invalid scale size lists fail closed instead of passing vacuously, and trapped offline network calls remain 0 |
