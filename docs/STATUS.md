...
| Security (injection/secrets/trust) | ✅ | adversarial benchmark: secret recall 1.0/precision 1.0 including credential-bearing database URLs and webhook URLs; explicit CLI/MCP memory writes refuse secrets in text and metadata before storage; high-entropy redaction covers regex-metacharacter tokens; 0 poison promoted across expanded attack families; 0 harmful capsule cards; deterministic fuzz cases. `eval security` plus `eval memory`/`eval mcp` guard it |
...
measured gains are recorded in the `memory` graph. Tests: 205, gate suites: 19 (`eval all` includes run/memory/promote/drift/retrieval/gate/security/branch/temporal/conflict/procedure/mcp/storage/telemetry/provenance/resilience/benchmarks/cli-docs-demo/quality)._
