...
| M4 — adapters + MCP | `adapters-mcp` | install per client + MCP contract + secure proxy | ✅ PASS (87/87; MCP=8 tools; 0 proxy leaks) |
...
| CLI / UX / docs / demo | ✅ | `eval cli-docs-demo` passes 11/11 and `eval memory` passes 16/16: help/docs command surface aligned, SPEC §15/§17 hook event drift fixed, demo facts stay memory-only/offline, doctor prints READY/NOT READY remediation, MCP advertises exactly 8 tools, Claude install round-trip and piped auto-detect hold, invalid numeric flags and invalid fact kinds fail before work starts, and typed CLI errors are action-oriented and stack-free |
