...
| M4 — adapters + MCP | `adapters-mcp` | install per client + MCP contract + secure proxy | ✅ PASS (84/84; MCP=8 tools; 0 proxy leaks) |
...
| MCP server & 8-tool surface | ✅ | `eval mcp` now covers 84/84 adapter/MCP checks: MCP 2025-11-25 initialize shape, exact 8-tool live/static surface with count-drift hard error, per-tool zod input + output-shape contracts including lifecycle-event enum and scalar JSON schema constraints, JSON-RPC -32602/-32601 errors, redacted tool/validation errors, bounded anti-repetition rider, last-8 provenance lookup, MCP `remember` git-anchor stamping and text/metadata secret refusal, MCP session-reference secret refusal before retrieval/planning/logging, telemetry precedence, real `serve --mcp` stdio initialize/tools-list, Claude adapter detection, static-floor secret refusal, and config-preserving/symlink-safe fail-closed adapter install/uninstall paths |
| Security (injection/secrets/trust) | ✅ | adversarial benchmark: secret recall 1.0/precision 1.0 including credential-bearing database URLs and webhook URLs; deterministic extraction skips symlinked directories and scans subjects before storage; LLM extraction scans subjects before storage; explicit CLI/MCP memory writes and open-loop session metadata refuse secrets before storage; high-entropy redaction covers regex-metacharacter tokens; 0 poison promoted across expanded attack families; 0 harmful capsule cards; deterministic fuzz cases. `eval security` plus `eval memory`/`eval mcp` guard it |
...
| Storage & migrations | ✅ | new `eval storage` passes 12/12: schema_version 4, reopen migrations 0, failed migrations roll back partial DDL and schema_version, v1→v4 rows preserved 3/3 with t_observed backfill, append-only expire tombstones retained, malformed rows skipped, missing optional ledger table degrades, WAL/FK/busy_timeout enforced, cascades/edge trail consistent |
...
| Resilience & fail-soft (I9) | ✅ | new `eval resilience` passes 15/15: no-key deterministic-only capsule emits with exit 0, corrupt DB, symlinked local stores, symlinked workspace config, and bad config degrade to empty output, missing git and planner crashes never propagate, secret-bearing hook payloads, hook session ids, prompts, transcript tails, and tool args are redacted before persistence or retrieval context, provider resolution returns `nullProvider`, no-key extraction is a deterministic no-op, and SessionStart/SessionEnd lifecycle hooks run fail-soft |
...
| CLI / UX / docs / demo | ✅ | `eval cli-docs-demo` passes 11/11: help/docs command surface aligned, SPEC §15/§17 hook event drift fixed, demo facts stay memory-only/offline, doctor prints READY/NOT READY remediation, MCP advertises exactly 8 tools, Claude install round-trip and piped auto-detect hold, invalid numeric flags fail before work starts, and typed CLI errors are action-oriented and stack-free |
...
measured gains are recorded in the `memory` graph. Tests: 229, gate suites: 19 (`eval all` includes run/memory/promote/drift/retrieval/gate/security/branch/temporal/conflict/procedure/mcp/storage/telemetry/provenance/resilience/benchmarks/cli-docs-demo/quality)._
