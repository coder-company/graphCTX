...
| Retrieval & ranking | ✅ | hybrid RRF + deterministic semantic features + bounded MMR diversity: recall@1/5/10 1.00, MRR 1.00, semantic no-overlap rank 1, SessionStart broad-push includes explicit user preferences, retriever suppresses send-unsafe facts before returning results, and fact tag updates keep FTS/vector indexes searchable. `eval retrieval` regression gate |
...
measured gains are recorded in the `memory` graph. Tests: 237, gate suites: 19 (`eval all` includes run/memory/promote/drift/retrieval/gate/security/branch/temporal/conflict/procedure/mcp/storage/telemetry/provenance/resilience/benchmarks/cli-docs-demo/quality)._
