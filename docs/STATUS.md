...
| Resilience & fail-soft (I9) | ✅ | new `eval resilience` passes 11/11: no-key deterministic-only capsule emits with exit 0, corrupt DB, symlinked local stores, and bad config degrade to empty output, missing git and planner crashes never propagate, secret-bearing hook payloads and hook session ids are redacted before episode persistence, provider resolution returns `nullProvider`, no-key extraction is a deterministic no-op, and SessionStart/SessionEnd lifecycle hooks run fail-soft |
...
measured gains are recorded in the `memory` graph. Tests: 223, gate suites: 19 (`eval all` includes run/memory/promote/drift/retrieval/gate/security/branch/temporal/conflict/procedure/mcp/storage/telemetry/provenance/resilience/benchmarks/cli-docs-demo/quality)._
