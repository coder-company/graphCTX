...
_Last updated: pre-budget precedence resolution. 190 tests, 19 gate suites green, all I1-I9 hold._

_Quality counters: Tests: 190. Gate suites: 19._
...
| Conflict & precedence | ✅ | comprehensive eval: 62-case ladder/determinism/resolve/reconcile/invalidation-precedence gate remains 62/62 with silentWrongWinners=0; planner now resolves precedence before budget redundancy so lower-precedence duplicate keys cannot hide structured evidence; real Runtime concurrent-writer stress over a shared store reports 3/3 races and `silentOverwrites: 0`. `eval conflict` guards it |
...
measured gains are recorded in the `memory` graph. Tests: 190, gate suites: 19 (`eval all` includes run/memory/promote/drift/retrieval/gate/security/branch/temporal/conflict/procedure/mcp/storage/telemetry/provenance/resilience/benchmarks/cli-docs-demo/quality)._
